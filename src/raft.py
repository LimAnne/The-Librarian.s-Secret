...
from utils.logconf import log_setup
...
from utils.format import DatasetConverter, datasetFormats, outputDatasetTypes
from pathlib import Path
from dotenv import load_dotenv
from utils.checkpointing import Checkpointing, checkpointed
import uuid
import shutil
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default="",
        help="If a file, the path at which the document is located. If a folder, the path at which to load all documents",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="The number of Q/A samples to generate after which to stop the generation process. Defaults to None, which means generating Q/A samples for all documents",
    )
...
def generate_chunk_instructions(
    chat_completer: ChatCompleter, chunk: Any, x=5, model: str = None
) -> list[str]:
    """
...
def encode_question_gen(
    question: str, chunk: Any, prompt_key: str = "gpt"
) -> list[str]:
    """
...
def generate_label(
    ...
    question = (
        ...
        if doctype == "api"
        else encode_question_gen(question, context, prompt_key)
    )
    response = chat_completer(
        model=model,
        messages=question,
...
def build_or_load_chunks(
    ...
    if not chunks:
        chunks = get_chunks(
            datapath, doctype, CHUNK_SIZE, OPENAPI_API_KEY, model=embedding_model
        )
...
def main():
    ...
    if auto_clean_checkpoints:
        logger.info(
            "Checkpoints will be automatically deleted after dataset generation. Remove --auto-clean-checkpoints to deactivate."
        )
...
def stage_generate(
    ...
    def generate_chunk_instructions_ds(
        chunk: str, chunk_id: int, doctype: str, *args, **kwargs
    ):
        """
        Generates a dataset of instructions for a given chunk.
        """
        questions = (
            generate_chunk_instructions(chunk=chunk, *args, **kwargs)
            if doctype == "api"
            else generate_instructions_gen(chunk=chunk, *args, **kwargs)
        )
    ...
    def generate_question_cot_answers(
        questions_ds, chunk_id: int, chunk: str, *args, **kwargs
    ):
    ...
    def process_chunk(i):
        ...
        chunk = chunks[i]
        questions_ds = generate_chunk_instructions_ds(
            chunk=chunk,
            chunk_id=i,
    ...
    if qa_threshold:
        logger.info(
            f"Will stop early as soon as the QA threshold is met: {qa_threshold}"
        )
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                if qa_threshold and gen_questions_count >= qa_threshold:
                    logger.info(
                        f"Met threshold {gen_questions_count} >= {qa_threshold} questions, stopping generation"
                    )
                    is_stopping.set()
                    break
